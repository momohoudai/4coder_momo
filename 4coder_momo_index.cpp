#ifndef FCODER_MOMO_INDEX_H
#define FCODER_MOMO_INDEX_H

enum Momo_Index_NoteKind
{
    Momo_Index_NoteKind_Null,
    Momo_Index_NoteKind_Scope,
    Momo_Index_NoteKind_Type,
    Momo_Index_NoteKind_Constant,
    Momo_Index_NoteKind_Function,
    Momo_Index_NoteKind_Decl,
    Momo_Index_NoteKind_Macro,
    Momo_Index_NoteKind_CommentTag,
    Momo_Index_NoteKind_CommentToDo,
    Momo_Index_NoteKind_COUNT
};

typedef u32 Momo_Index_NoteFlags;
enum
{
    Momo_Index_NoteFlag_Prototype   = (1<<0),
    Momo_Index_NoteFlag_ProductType = (1<<1),
    Momo_Index_NoteFlag_SumType     = (1<<2),
};
...
struct Momo_Index_Note
{
    ...
    Momo_Index_NoteKind kind;
    Momo_Index_NoteFlags flags;
    Range_i64 range;
...
typedef u32 Momo_Index_TokenSkipFlags;
enum
{
    Momo_Index_TokenSkipFlag_SkipWhitespace = (1<<0),
};
...
internal Momo_Index_Note *Momo_Index_MakeNote(Application_Links *app,
                                          ...
                                          Momo_Index_Note *parent,
                                          String_Const_u8 string,
                                          Momo_Index_NoteKind note_kind,
                                          Momo_Index_NoteFlags note_flags,
                                          Range_i64 range);
internal void Momo_Index_ParseFile(Application_Links *app, Momo_Index_File *file, String_Const_u8 string, Token_Array tokens);
internal b32 Momo_Index_ParseCtx_Inc(Momo_Index_ParseCtx *ctx, Momo_Index_TokenSkipFlags flags);
#define Momo_Index_ParseCtx_IncWs(ctx) Momo_Index_ParseCtx_Inc(ctx, Momo_Index_TokenSkipFlag_SkipWhitespace)
internal b32 Momo_Index_RequireToken(Momo_Index_ParseCtx *ctx, String_Const_u8 string, Momo_Index_TokenSkipFlags flags);
internal b32 Momo_Index_RequireTokenKind(Momo_Index_ParseCtx *ctx, Token_Base_Kind kind, Token **token_out, Momo_Index_TokenSkipFlags flags);
internal b32 Momo_Index_RequireTokenSubKind(Momo_Index_ParseCtx *ctx, int sub_kind, Token **token_out, Momo_Index_TokenSkipFlags flags);
...
// %n -> note,          requires Momo_Index_NoteKind and Momo_Index_Note ** for output note
...
{
    Momo_Index_Lock();
    Momo_Language *lang = Momo_LanguageFromBuffer(app, file->buffer);
...
internal b32
Momo_Index_ParseCtx_Inc(Momo_Index_ParseCtx *ctx, Momo_Index_TokenSkipFlags flags)
{
    if(flags & Momo_Index_TokenSkipFlag_SkipWhitespace)
    {
...
internal b32
Momo_Index_RequireToken(Momo_Index_ParseCtx *ctx, String_Const_u8 string, Momo_Index_TokenSkipFlags flags)
{
    b32 result = 0;
...
internal b32
Momo_Index_RequireTokenKind(Momo_Index_ParseCtx *ctx, Token_Base_Kind kind, Token **token_out, Momo_Index_TokenSkipFlags flags)
{
    b32 result = 0;
    Token *token = token_it_read(&ctx->it);
    if(token)
...
internal b32
Momo_Index_RequireTokenSubKind(Momo_Index_ParseCtx *ctx, int sub_kind, Token **token_out, Momo_Index_TokenSkipFlags flags)
{
    b32 result = 0;
    Token *token = token_it_read(&ctx->it);
    if(token)
...
Momo_Index_ParseComment(Momo_Index_ParseCtx *ctx, Token *token)
{
    ...
    for(u64 i = 0; i < string.size; i += 1)
    {
        if(string.str[i] == '@')
        {
            Momo_Index_MakeNote(ctx->app, ctx->file, 0, string_substring(string, Ii64(i, string.size)), Momo_Index_NoteKind_CommentTag, 0, Ii64(token));
            break;
        }
        else if(i+4 < string.size && string_match(S8Lit("TODO"), string_substring(string, Ii64(i, i + 4))))
        {
            Momo_Index_MakeNote(ctx->app, ctx->file, 0, string_substring(string, Ii64(i, string.size)), Momo_Index_NoteKind_CommentToDo, 0, Ii64(token));
        }
    }
}
...
Momo_Index_SkipOpTokens(Momo_Index_ParseCtx *ctx)
{
    ...
    for(;!ctx->done;)
    {
        ...
        Momo_Index_ParseCtx_Inc(ctx, Momo_Index_TokenSkipFlag_SkipWhitespace);
    }
}
...
Momo_Index_ParsePattern(Momo_Index_ParseCtx *ctx, char *fmt, ...)
{
    ...
    Momo_Index_TokenSkipFlags flags = Momo_Index_TokenSkipFlag_SkipWhitespace;
    ...
    for(int i = 0; fmt[i];)
    {
        if(fmt[i] == '%')
        {
            switch(fmt[i+1])
            {
                ...
                case 'n':
                {
                    Momo_Index_NoteKind kind = va_arg(args, Momo_Index_NoteKind);
