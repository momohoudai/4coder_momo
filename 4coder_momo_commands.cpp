...
CUSTOM_COMMAND_SIG(momo_change_active_panel_left)
CUSTOM_DOC("Cycle panel left") {
  View_ID active_view = get_active_view(app, Access_ReadVisible);
  Rect_f32 active_rect = view_get_screen_rect(app, active_view);
  
  
  // find all panels
  View_ID found_view = active_view;
  f32 best_diff = max_f32;
  ...
  view_set_active(app, found_view); 
  
}

CUSTOM_COMMAND_SIG(momo_change_active_panel_right)
CUSTOM_DOC("Cycle panel right") {
  View_ID active_view = get_active_view(app, Access_ReadVisible);
  Rect_f32 active_rect = view_get_screen_rect(app, active_view);
...
CUSTOM_COMMAND_SIG(momo_change_active_panel_up)
CUSTOM_DOC("Cycle panel up") {
...
CUSTOM_COMMAND_SIG(momo_change_active_panel_down)
CUSTOM_DOC("Cycle panel down") {
  View_ID active_view = get_active_view(app, Access_ReadVisible);
  Rect_f32 active_rect = view_get_screen_rect(app, active_view);
  ...
  if (buffer != 0) {
    ...
    for (;;) {
      ...
      if (match_key_code(&in, KeyCode_V)) {
        ...
        momo_change_active_panel_left(app);
        break;
      }
      else if (match_key_code(&in, KeyCode_L)) {
        momo_change_active_panel_right(app);
        break;
      }
      else if (match_key_code(&in, KeyCode_K)) {
        momo_change_active_panel_up(app);
        break;
      }
...
CUSTOM_COMMAND_SIG(momo_open_matching_file_cpp)
